...
def paciente_dashboard(request):
    ...
    turnos_futuros = Turno.objects.filter(
        paciente=paciente, fecha__gte=date.today(), estado__in=["pendiente", "confirmado"]
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            existe = (
                ...
                .exclude(pk=turno.pk if turno.pk else None)
                .exists()
            )
...
def calendario_turnos_view(request):
    ...
    turnos_query = Turno.objects.filter(
        fecha__gte=primer_dia, fecha__lte=ultimo_dia, estado__in=["pendiente", "confirmado"]
    )
        ...
        for disp in disponibilidades:
            ...
            for hora in horarios:
                ocupado = Turno.objects.filter(
                    medico=medico, fecha=fecha, hora=hora, estado__in=["pendiente", "confirmado"]
                ).exists()
