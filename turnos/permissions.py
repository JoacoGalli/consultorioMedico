...
def secretaria_required(function=None, redirect_field_name="next", login_url="/login/"):
    ...
    actual_decorator = user_passes_test(
        es_secretaria, login_url=login_url, redirect_field_name=redirect_field_name
    )
    if function:
...
def paciente_required(function=None, redirect_field_name="next", login_url="/login/"):
    ...
    """
    actual_decorator = user_passes_test(
        es_paciente, login_url=login_url, redirect_field_name=redirect_field_name
    )
    if function:
        return actual_decorator(function)
    return actual_decorator
...
def verificar_permiso_turno(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if turno_id:
            ...
            except Turno.DoesNotExist as err:
                raise PermissionDenied("El turno no existe") from err
