...
from .models import Paciente, Medico, Cobertura, DisponibilidadMedico, Turno
...
class ModelsIntegrationTest(TestCase):
    ...
    def test_relacion_paciente_cobertura_medico(self):
        ...
        self.assertTrue(
            self.medico.coberturas.filter(id=self.paciente.cobertura.id).exists()
        )
    ...
    def test_disponibilidad_genera_turnos(self):
        ...
        self.assertTrue(Turno.objects.filter(
            medico=self.medico,
            fecha=fecha_futura,
            hora=horarios[0]
        ).exists())
